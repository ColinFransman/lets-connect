    ...
    <body>
        ...
        <script>
            ...
            function drop(ev) {
                ...
                // Haal het id van de workshop op (bijv. "workshop1")
                const workshopId = parseInt(draggedElement.id.replace('workshop', ''));

                // Controleer of de workshop op de juiste ronde wordt geplaatst
                if (ev.target.classList.contains("round")) {
                    const targetRoundId = parseInt(ev.target.id);

                    // Check of de workshop bij de juiste ronde kan worden geplaatst
                    if (
                        (workshopId === 1 && targetRoundId === 1) ||
                        (workshopId === 2 && targetRoundId === 2) ||
                        (workshopId === 3 && targetRoundId === 3) ||
                        (workshopId >= 4 && workshopId <= 12) // Workshops 4 t/m 12 kunnen overal
                    ) {
